...
+ (BOOL)needsDisplayForKey:(NSString *)key
{
    if ([key isEqualToString:@"progressTintColor"]
        || [key isEqualToString:@"groundTintColor"]
...
- (id)initWithLayer:(id)layer
{
    ...
    if (self)
    {
        if ([layer isKindOfClass:[HSLearnCircleProgressLayer class]])
        {
            ...
            self.groundTintColor = other.groundTintColor;
...
{
    // 画一个灰色的底
    CGContextSetFillColorWithColor(context, self.groundTintColor.CGColor);
    CGContextAddArc(context, aCenter.x, aCenter.y, aRadius, 0.0f, TWO_PI, 0);
    if (!_isFillBackGround) {
        CGContextAddArc(context, aCenter.x, aCenter.y, aRadius * (1.0f - aFillRadius), TWO_PI, 0.0f, 1);
    }
    CGPathDrawingMode drawingMode = kCGPathFill;
    CGContextClosePath(context);
    CGContextDrawPath(context, drawingMode);
